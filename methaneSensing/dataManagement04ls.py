...
for experiment_name, exp_info in experiments.items():
   ...
   # Which features are actually present?
    ...
    dt_seconds = float(pd.Series(dfLag.index).diff().dropna().dt.total_seconds().median())
    ...
    for feat in present_features:
        lag_s, lag_k, peak = _lag_vs_target(dfLag, feat, target_column, dt_seconds, MAX_LAG_SECONDS)
        results.append({
            "feature": feat,
